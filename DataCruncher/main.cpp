...
TEST_CASE("constructors, getters, setters") {
    ...
    SECTION("getters / setters") {
            SECTION("getValueAt handles index errors") {
                DataCruncher dataC(3);
                dataC[0] = 1;
                dataC[1] = 2;
                dataC[2] = 3;
                REQUIRE(dataC.getValueAt(-1)==1);
                REQUIRE(dataC.getValueAt(-12345)==1);
                REQUIRE(dataC.getValueAt(0)==1);
                REQUIRE(dataC.getValueAt(1)==2);
                REQUIRE(dataC.getValueAt(2)==3);
                REQUIRE(dataC.getValueAt(3)==3);
                REQUIRE(dataC.getValueAt(12345)==3);
            }
    }
}
